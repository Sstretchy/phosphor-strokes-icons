import { createIcon } from "../create-icon.js";

export const OfficeChair = createIcon("office-chair", [
  [
    "path",
    {
      d: "M5.625 18.125C5.625 17.6278 5.82254 17.1508 6.17418 16.7992C6.52581 16.4476 7.00275 16.25 7.5 16.25H12.5C12.9972 16.25 13.4742 16.4476 13.8258 16.7992C14.1774 17.1508 14.375 17.6278 14.375 18.125",
      stroke: "currentColor",
      strokeLinecap: "round",
      strokeLinejoin: "round"
    }
  ],
  [
    "path",
    {
      d: "M10 13.125V18.125",
      stroke: "currentColor",
      strokeLinecap: "round",
      strokeLinejoin: "round"
    }
  ],
  [
    "path",
    {
      d: "M1.25 10H3.125C3.125 10.8288 3.45424 11.6237 4.04029 12.2097C4.62634 12.7957 5.4212 13.125 6.25 13.125H13.75C14.5788 13.125 15.3737 12.7957 15.9597 12.2097C16.5457 11.6237 16.875 10.8288 16.875 10H18.75",
      stroke: "currentColor",
      strokeLinecap: "round",
      strokeLinejoin: "round"
    }
  ],
  [
    "path",
    {
      d: "M7.32188 1.875C7.17144 1.87499 7.026 1.92928 6.91231 2.02787C6.79869 2.12647 6.72438 2.26276 6.70313 2.41172L5.63127 9.91175C5.61862 10.0004 5.62516 10.0907 5.65046 10.1766C5.67576 10.2624 5.71921 10.3419 5.77788 10.4096C5.83656 10.4772 5.90908 10.5314 5.99053 10.5686C6.07199 10.6057 6.16048 10.625 6.25 10.625H13.75C13.8396 10.625 13.9281 10.6057 14.0095 10.5686C14.0909 10.5314 14.1635 10.4772 14.2221 10.4096C14.2808 10.3419 14.3242 10.2624 14.3496 10.1766C14.3749 10.0907 14.3814 10.0004 14.3687 9.91175L13.2969 2.41172C13.2756 2.26276 13.2014 2.12647 13.0877 2.02787C12.974 1.92928 12.8286 1.87499 12.6781 1.875H7.32188Z",
      stroke: "currentColor",
      strokeLinecap: "round",
      strokeLinejoin: "round"
    }
  ]
], {
  viewBox: "0 0 20 20",
  absoluteStrokeBase: 20
});

export default OfficeChair;
