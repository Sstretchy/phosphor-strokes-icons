import { createIcon } from "../create-icon.js";

export const PersonArmsSpread = createIcon("person-arms-spread", [
  [
    "path",
    {
      d: "M10 5C11.0356 5 11.875 4.16053 11.875 3.125C11.875 2.08947 11.0356 1.25 10 1.25C8.96444 1.25 8.125 2.08947 8.125 3.125C8.125 4.16053 8.96444 5 10 5Z",
      stroke: "currentColor",
      strokeLinecap: "round",
      strokeLinejoin: "round"
    }
  ],
  [
    "path",
    {
      d: "M3.04709 8.03906C2.1338 7.61719 2.4338 6.25 3.43771 6.25H16.5627C17.5682 6.25 17.8682 7.61719 16.9533 8.03906L12.5002 10L14.2869 16.7914C14.3878 17.0161 14.3962 17.2715 14.3102 17.5024C14.2244 17.7332 14.0511 17.9209 13.8278 18.0251C13.6046 18.1291 13.3493 18.1412 13.1172 18.0586C12.8852 17.9759 12.695 17.8053 12.5877 17.5836L10.0002 13.125L7.41268 17.5836C7.30543 17.8053 7.11525 17.9759 6.88319 18.0586C6.65112 18.1412 6.39587 18.1291 6.17264 18.0251C5.94939 17.9209 5.77607 17.7332 5.69015 17.5024C5.60423 17.2715 5.61262 17.0161 5.71349 16.7914L7.50019 10L3.04709 8.03906Z",
      stroke: "currentColor",
      strokeLinecap: "round",
      strokeLinejoin: "round"
    }
  ]
], {
  viewBox: "0 0 20 20",
  absoluteStrokeBase: 20
});

export default PersonArmsSpread;
