import { createIcon } from "../create-icon.js";

export const FolderSimpleLock = createIcon("folder-simple-lock", [
  [
    "path",
    {
      d: "M14 26H5C4.73478 26 4.48043 25.8946 4.29289 25.7071C4.10536 25.5196 4 25.2652 4 25V8C4 7.73478 4.10536 7.48043 4.29289 7.29289C4.48043 7.10536 4.73478 7 5 7H11.6663C11.8826 7 12.0932 7.07018 12.2662 7.2L15.7338 9.8C15.9068 9.92982 16.1174 10 16.3337 10H27C27.2652 10 27.5196 10.1054 27.7071 10.2929C27.8946 10.4804 28 10.7348 28 11V13",
      stroke: "currentColor",
      strokeLinecap: "round",
      strokeLinejoin: "round"
    }
  ],
  [
    "path",
    {
      d: "M28 21H19V26H28V21Z",
      stroke: "currentColor",
      strokeLinecap: "round",
      strokeLinejoin: "round"
    }
  ],
  [
    "path",
    {
      d: "M21 21V19.5C21 18.837 21.2634 18.2011 21.7322 17.7322C22.2011 17.2634 22.837 17 23.5 17C24.163 17 24.7989 17.2634 25.2678 17.7322C25.7366 18.2011 26 18.837 26 19.5V21",
      stroke: "currentColor",
      strokeLinecap: "round",
      strokeLinejoin: "round"
    }
  ]
]);

export default FolderSimpleLock;
