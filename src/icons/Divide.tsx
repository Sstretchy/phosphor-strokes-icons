import { createIcon } from "../create-icon.js";

export const Divide = createIcon("divide", [
  [
    "path",
    {
      d: "M5 16H27",
      stroke: "currentColor",
      strokeLinecap: "round",
      strokeLinejoin: "round"
    }
  ],
  [
    "path",
    {
      d: "M16 9.75C16.9665 9.75 17.75 8.9665 17.75 8C17.75 7.0335 16.9665 6.25 16 6.25C15.0335 6.25 14.25 7.0335 14.25 8C14.25 8.9665 15.0335 9.75 16 9.75Z",
      fill: "currentColor",
      stroke: "none"
    }
  ],
  [
    "path",
    {
      d: "M16 25.75C16.9665 25.75 17.75 24.9665 17.75 24C17.75 23.0335 16.9665 22.25 16 22.25C15.0335 22.25 14.25 23.0335 14.25 24C14.25 24.9665 15.0335 25.75 16 25.75Z",
      fill: "currentColor",
      stroke: "none"
    }
  ]
]);

export default Divide;
